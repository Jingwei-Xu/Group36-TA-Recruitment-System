...
public class RegisterPagePanel extends JPanel {
    private static final int CARD_CONTENT_WIDTH = 560;
    ...
    public RegisterPagePanel(AppFrame app) {
        ...
        card.setBorder(new EmptyBorder(32, 44, 36, 44));
        card.setAlignmentX(Component.CENTER_ALIGNMENT);
        card.setMaximumSize(new Dimension(CARD_CONTENT_WIDTH + 88, Integer.MAX_VALUE));

        JLabel heroIcon = new JLabel(new PortalIcon("plus-user", PRIMARY_PURPLE, 36));
        heroIcon.setAlignmentX(Component.CENTER_ALIGNMENT);
        JPanel heroTile = iconCircle(heroIcon, 62);
        ...
        userField = styledField("Enter your username");
        passField = styledPasswordField("Enter your password");
        confirmField = styledPasswordField("Confirm your password");

        JPanel userRow = createInlineLabeledField(new PortalIcon("user", PRIMARY_PURPLE, 20), "Username", userField);
        JPanel passRow = createInlinePasswordRow(new PortalIcon("lock", PRIMARY_PURPLE, 20), "Password", passField);
        JPanel confirmRow = createInlinePasswordRow(new PortalIcon("lock", PRIMARY_PURPLE, 20), "Confirm Password", confirmField);

        JLabel hint = new JLabel("At least 8 characters, including letters and numbers", SwingConstants.CENTER);
        hint.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        hint.setForeground(new Color(0x7A8496));
        hint.setAlignmentX(Component.CENTER_ALIGNMENT);
        hint.setHorizontalAlignment(SwingConstants.CENTER);
        hint.setMaximumSize(new Dimension(CARD_CONTENT_WIDTH, 40));
        ...
        roleCombo.setBorder(BorderFactory.createEmptyBorder(4, 12, 4, 8));
        roleCombo.setUI(new BasicComboBoxUI() {
            @Override
            protected JButton createArrowButton() {
                JButton b = new JButton(new PortalIcon("chevron", MUTED_TEXT, 14));
        ...
        JPanel roleRow = createInlineRoleRow(new PortalIcon("shield", PRIMARY_PURPLE, 20), "Role", roleCombo);

        GradientButton register = new GradientButton("Register", new PortalIcon("plus-user", Color.WHITE, 16));
        register.setAlignmentX(Component.CENTER_ALIGNMENT);
        register.setMaximumSize(new Dimension(CARD_CONTENT_WIDTH, 44));
        register.setPreferredSize(new Dimension(CARD_CONTENT_WIDTH, 44));
    ...
    private JPanel createInlineRoleRow(Icon icon, String labelText, JComboBox<String> combo) {
        JPanel row = new JPanel(new BorderLayout(LABEL_FIELD_GAP, 0));
        row.setOpaque(false);
        row.setAlignmentX(Component.CENTER_ALIGNMENT);
        row.setMaximumSize(new Dimension(CARD_CONTENT_WIDTH, INPUT_ROW_HEIGHT + 8));
        ...
        JPanel shellInner = new JPanel(new BorderLayout());
        shellInner.setOpaque(false);
        shellInner.add(combo, BorderLayout.CENTER);
        row.add(fieldShellStretch(shellInner, INPUT_ROW_HEIGHT), BorderLayout.CENTER);
        return row;
    }
    ...
    private JPanel underline() {
        ...
        u.setPreferredSize(new Dimension(32, 3));
        u.setMinimumSize(new Dimension(32, 3));
        u.setMaximumSize(new Dimension(32, 3));
        return u;
    }
    ...
    private static class HintTextField extends JTextField {
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (getText().isEmpty() && !isFocusOwner()) {
                ...
                g2.drawString(hint, 14, (getHeight() - fm.getHeight()) / 2 + fm.getAscent());
                g2.dispose();
            }
        }
    }
    ...
    private static class PortalIcon implements Icon {
        ...
        public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            switch (type) {
                ...
                case "plus-user" -> paintPlusUser(g2, ox, oy, w, h, stroke, color);
                case "shield" -> paintShield(g2, ox, oy, w, h, stroke);
                case "chevron" -> paintChevron(g2, ox, oy, w, h, stroke);
        ...
        private static void paintLock(Graphics2D g2, float ox, float oy, float w, float h, float stroke) {
            ...
            g2.draw(new Arc2D.Float(shX, shY, shW, shH, 0, 180, Arc2D.OPEN));
        ...
        private static void paintEyeOpen(Graphics2D g2, float ox, float oy, float w, float h, float stroke, Color tint) {
            ...
            g2.fill(new Ellipse2D.Float(cx + pr * 0.35f, cy - pr * 0.35f, pr * 0.55f, pr * 0.55f));
            g2.setStroke(new BasicStroke(stroke, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        }
        ...
        /** User bust + rounded plus badge (add account). Light ink = stroke-only for purple buttons. */
        private static void paintPlusUser(Graphics2D g2, float ox, float oy, float w, float h, float stroke, Color ink) {
            float s = Math.min(w, h);
            float cx = ox + w * 0.4f;
            float headR = s * 0.16f;
            float headCy = oy + h * 0.32f;
            GeneralPath shoulders = new GeneralPath();
            float left = ox + w * 0.12f;
            float right = ox + w * 0.62f;
            float bottomY = oy + h * 0.82f;
            shoulders.moveTo(left, bottomY);
            shoulders.quadTo(cx, headCy + headR * 1.1f, right, bottomY);
            ...
            if (lightInk) {
                ...
                g2.draw(new Ellipse2D.Float(cx - headR, headCy - headR, headR * 2, headR * 2));
                g2.draw(shoulders);
                g2.draw(new Ellipse2D.Float(bx, by, br, br));
                g2.setStroke(new BasicStroke(stroke * 1.2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
                g2.draw(new Line2D.Float(hc - hl, vc, hc + hl, vc));
                g2.draw(new Line2D.Float(hc, vc - hl, hc, vc + hl));
            } else {
                g2.setColor(ink);
                g2.draw(new Ellipse2D.Float(cx - headR, headCy - headR, headR * 2, headR * 2));
                g2.draw(shoulders);
                g2.fill(new Ellipse2D.Float(bx, by, br, br));
                g2.setColor(new Color(255, 255, 255, 248));
                g2.setStroke(new BasicStroke(stroke * 1.15f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
                g2.draw(new Line2D.Float(hc - hl, vc, hc + hl, vc));
                g2.draw(new Line2D.Float(hc, vc - hl, hc, vc + hl));
                g2.setColor(ink);
            }
        ...
        /** Heraldic shield with spine highlight. */
        private static void paintShield(Graphics2D g2, float ox, float oy, float w, float h, float stroke) {
            float cx = ox + w / 2;
            float top = oy + h * 0.12f;
            float left = ox + w * 0.18f;
            float right = ox + w * 0.82f;
            float midY = oy + h * 0.42f;
            float bot = oy + h * 0.9f;
            GeneralPath sh = new GeneralPath();
            sh.moveTo(cx, top);
            sh.curveTo(right, oy + h * 0.22f, right, midY, cx, bot);
            sh.curveTo(left, midY, left, oy + h * 0.22f, cx, top);
            g2.draw(sh);
            g2.setStroke(new BasicStroke(stroke * 0.55f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            g2.draw(new Line2D.Float(cx, top + h * 0.08f, cx, bot - h * 0.08f));
            g2.setStroke(new BasicStroke(stroke, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        }

        /** Rounded chevron pointing down. */
        private static void paintChevron(Graphics2D g2, float ox, float oy, float w, float h, float stroke) {
            float cx = ox + w / 2;
            float ty = oy + h * 0.35f;
            float by = oy + h * 0.72f;
            float inset = w * 0.22f;
            GeneralPath p = new GeneralPath();
            p.moveTo(ox + inset, ty);
            p.quadTo(cx, by + h * 0.06f, ox + w - inset, ty);
            g2.draw(p);
        }
    }
}
