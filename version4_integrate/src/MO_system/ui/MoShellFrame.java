...
public final class MoShellFrame extends JFrame implements MoShellHost {
    ...
    private void performLogout() {
        ...
        if (createJobPanel != null && createJobPanel.isShowing() && !createJobPanel.confirmLeaveForNavigation()) {
            return;
        }
    ...
    public void showJobDetail(Job job) {
        ...
        } else if (!jobDetailPanel.representsJob(j)) {
            jobDetailPanel.setJob(j);
        }
        jobDetailPanel.ensureScrollTop();
        updateNavHighlight(NavigationPanel.Tab.JOB_MANAGEMENT);
    ...
    private Job resolveJob(Job job) {
        ...
        List<Job> all = jobRepository.loadJobsForMo(MoContext.getCurrentMoUserId());
        for (Job j : all) {
