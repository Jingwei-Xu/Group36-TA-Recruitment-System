package Authentication_Module.view;

import Authentication_Module.session.SessionManager;
import Authentication_Module.model.User;
import profile_module.data.JsonStore;
import profile_module.data.ProfileData;
import TA_Job_Application_Module.model.TAUser;
import TA_Job_Application_Module.service.DataService;

import com.formdev.flatlaf.FlatLightLaf;

import javax.swing.*;
import java.awt.*;


public class TAHomePagePanel extends JPanel {

    private final AppFrame app;
    private final JLabel label;

    public TAHomePagePanel(AppFrame app) {
        this.app = app;
        setLayout(new BorderLayout());
        setBackground(Color.WHITE);

        label = new JLabel("TA Portal - Loading...", SwingConstants.CENTER);
        label.setFont(new Font("Arial", Font.PLAIN, 16));
        add(label, BorderLayout.CENTER);
    }

    @Override
    public void setVisible(boolean aFlag) {
        super.setVisible(aFlag);
        if (aFlag) {
            SwingUtilities.invokeLater(this::tryOpenProfilePortal);
        }
    }

    private void tryOpenProfilePortal() {
        // If the user has already logged out, do not open the portal.
        if (SessionManager.getCurrentUser() == null) {
            return;
        }
        if (!isShowing()) {
            return;
        }
        // Re-check logout state (double-check).
        if (SessionManager.getCurrentUser() == null) {
            return;
        }

        User authUser = SessionManager.getCurrentUser();
        if (authUser == null) {
            label.setText("No user logged in. Please login first.");
            return;
        }

        TAUser taUser = DataService.getInstance().loadUserData(authUser.getUsername(), authUser.getRole());
        ProfileData profile = buildProfileFromAuthUser(authUser, taUser);

        JsonStore store = new JsonStore(authUser.getUsername());
        ProfileData fromDisk = store.load();
        if (fromDisk != null) {
            mergeLocalProfileSnapshot(profile, fromDisk);
        }

        SwingUtilities.invokeLater(() -> {
            
            try {
                UIManager.setLookAndFeel(new FlatLightLaf());
            } catch (Exception e) {
                try {
                    UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
                } catch (Exception ignored) {}
            }

           
            String initialRoute = (taUser != null && taUser.isOnboardingCompleted())
                    ? profile_module.ui.AppFrame.ROUTE_DASHBOARD
                    : profile_module.ui.AppFrame.ROUTE_ONBOARDING;

            profile_module.ui.AppFrame profileFrame = new profile_module.ui.AppFrame(store, profile, initialRoute);
            profileFrame.setAuthenticatedContext(authUser.getUsername(), authUser.getRole());
            profileFrame.setOnLogout(() -> {
                profileFrame.dispose();
                SessionManager.logout();
                app.setVisible(true);
                app.showPage("HOME");
            });
            profileFrame.setVisible(true);
        });

        
        app.setVisible(false);
    }

    
    private ProfileData buildProfileFromAuthUser(User user, TAUser taUser) {
        ProfileData p = new ProfileData();
        if (taUser != null) {
            p.fullName = taUser.getProfile() != null ? taUser.getProfile().getFullName() : user.getUsername();
            p.studentId = taUser.getProfile() != null ? taUser.getProfile().getStudentId() : "";
            p.year = taUser.getProfile() != null ? taUser.getProfile().getYear() : "";
            p.programMajor = taUser.getProfile() != null ? taUser.getProfile().getProgramMajor() : "";
            p.email = taUser.getAccount() != null ? taUser.getAccount().getEmail() : user.getUsername() + "@university.edu";
            p.phoneNumber = taUser.getProfile() != null ? taUser.getProfile().getPhoneNumber() : "";
            p.address = taUser.getProfile() != null ? taUser.getProfile().getAddress() : "";
            p.shortBio = taUser.getProfile() != null ? taUser.getProfile().getShortBio() : "";
            
            if (taUser.getSkills() != null) {
                p.skills = new java.util.ArrayList<>();
                // Prefer the new taSkillPool structure so all selected skills survive restart/login.
                if (taUser.getSkills().getTaSkillPool() != null) {
                    TAUser.TaSkillPool pool = taUser.getSkills().getTaSkillPool();
                    if (pool.getTechnicalSkills() != null) {
                        copySelectedSkillItems(
                                p.skills,
                                pool.getTechnicalSkills().getProgrammingAndSoftwareFundamentals(),
                                "Programming Languages"
                        );
                        copySelectedSkillItems(
                                p.skills,
                                pool.getTechnicalSkills().getHardwareAndLogicDesign(),
                                "Hardware & Logic Design"
                        );
                        copySelectedSkillItems(
                                p.skills,
                                pool.getTechnicalSkills().getEmbeddedSystemsAndLowLevelDevelopment(),
                                "Embedded Systems"
                        );
                    }
                    if (pool.getEngineeringAndTools() != null) {
                        copySelectedSkillItems(
                                p.skills,
                                pool.getEngineeringAndTools().getProfessionalDevelopmentAndSimulationTools(),
                                "Professional Development & Simulation Tools"
                        );
                    }
                    if (pool.getLanguageAndCommunication() != null) {
                        copySelectedSkillItems(
                                p.skills,
                                pool.getLanguageAndCommunication().getCrossCulturalCommunication(),
                                "English"
                        );
                    }
                } else {
                    // Legacy fallback.
                    copySkills(p.skills, taUser.getSkills().getProgramming());
                    copySkills(p.skills, taUser.getSkills().getTeaching());
                    copySkills(p.skills, taUser.getSkills().getCommunication());
                    copySkills(p.skills, taUser.getSkills().getOther());
                }
            }
           
            if (taUser.getCv() != null) {
                p.cv = new profile_module.data.CvInfo();
                p.cv.fileName = taUser.getCv().getOriginalFileName() != null ? taUser.getCv().getOriginalFileName() : "";
                p.cv.status = taUser.getCv().isUploaded() ? "Uploaded" : "";
                p.cv.lastUpdated = taUser.getCv().getUploadedAt() != null ? taUser.getCv().getUploadedAt() : "";
                p.cv.sizeLabel = "";
            }
            p.profileCompletionPercent = taUser.getProfileCompletion();
            p.numberOfApplications = taUser.getApplicationSummary() != null ? taUser.getApplicationSummary().getTotalApplications() : 0;
        } else {
            p.fullName = user.getUsername();
            p.studentId = "";
            p.email = user.getUsername() + "@university.edu";
            p.profileCompletionPercent = 0;
            p.numberOfApplications = 0;
        }
        return p;
    }

   
    private void copySkills(java.util.List<profile_module.data.SkillItem> target, java.util.List<TAUser.Skill> source) {
        if (source != null) {
            for (TAUser.Skill s : source) {
                profile_module.data.SkillItem item = new profile_module.data.SkillItem();
                item.name = s.getName();
                item.category = determineCategory(s);
                item.proficiency = s.getProficiency();
                target.add(item);
            }
        }
    }

    private void copySelectedSkillItems(
            java.util.List<profile_module.data.SkillItem> target,
            java.util.List<TAUser.SkillItem> source,
            String category
    ) {
        if (source == null) return;
        for (TAUser.SkillItem s : source) {
            if (s == null || !s.isSelected()) continue;
            profile_module.data.SkillItem item = new profile_module.data.SkillItem();
            item.name = s.getName();
            item.category = category;
            item.proficiency = s.getProficiency();
            target.add(item);
        }
    }

   
    private String determineCategory(TAUser.Skill s) {
        
        return "Other Skills";
    }

    /**
     * Profile module also persists under {@link JsonStore} (user home, {@code .ta-system-swing/<username>/profile.json}).
     * Fills in TA-backed fields when the server file is missing data but the last profile-module session had values
     * (e.g. sync failed or older bugs).
     */
    private static void mergeLocalProfileSnapshot(ProfileData primary, ProfileData fromDisk) {
        if (blank(primary.fullName) && !blank(fromDisk.fullName)) {
            primary.fullName = fromDisk.fullName;
        }
        if (blank(primary.studentId) && !blank(fromDisk.studentId)) {
            primary.studentId = fromDisk.studentId;
        }
        if (blank(primary.year) && !blank(fromDisk.year)) {
            primary.year = fromDisk.year;
        }
        if (blank(primary.programMajor) && !blank(fromDisk.programMajor)) {
            primary.programMajor = fromDisk.programMajor;
        }
        if (blank(primary.email) && !blank(fromDisk.email)) {
            primary.email = fromDisk.email;
        }
        if (blank(primary.phoneNumber) && !blank(fromDisk.phoneNumber)) {
            primary.phoneNumber = fromDisk.phoneNumber;
        }
        if (blank(primary.address) && !blank(fromDisk.address)) {
            primary.address = fromDisk.address;
        }
        if (blank(primary.shortBio) && !blank(fromDisk.shortBio)) {
            primary.shortBio = fromDisk.shortBio;
        }
        if (primary.cv != null && fromDisk.cv != null) {
            if (blank(primary.cv.fileName) && !blank(fromDisk.cv.fileName)) {
                primary.cv.fileName = fromDisk.cv.fileName;
                primary.cv.status = fromDisk.cv.status != null ? fromDisk.cv.status : "";
                primary.cv.lastUpdated = fromDisk.cv.lastUpdated != null ? fromDisk.cv.lastUpdated : "";
                primary.cv.sizeLabel = fromDisk.cv.sizeLabel != null ? fromDisk.cv.sizeLabel : "";
            }
        }
        if ((primary.skills == null || primary.skills.isEmpty())
                && fromDisk.skills != null && !fromDisk.skills.isEmpty()) {
            primary.skills = new java.util.ArrayList<>(fromDisk.skills);
        }
        primary.recomputeCompletion();
    }

    private static boolean blank(String s) {
        return s == null || s.trim().isEmpty();
    }
}
