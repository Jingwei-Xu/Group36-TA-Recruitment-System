...
public final class AppFrame extends JFrame {
    ...
    // Preserve the original Look and Feel for restoration on exit.
    ...
    public AppFrame(JsonStore store, ProfileData initialProfile, String initialRoute) {
        ...
        try {
            // Save current Look and Feel to restore on exit.
    ...
    public void setAuthenticatedContext(String username, String role) {
        ...
        // Constructor already called showRoute(initialRoute) before this method runs, so the first
        // dashboard.refresh() used the previous DataService currentUser. Re-sync all profile UI.
        dashboard.refresh();
        profileScreen.refresh();
        editProfileScreen.refresh();
        editSkillsScreen.refresh();
    ...
    public void showRoute(String route) {
        // Restore the original Look and Feel when returning from TA Portal to Dashboard.
    ...
    public void openJobApplicationPortal(String initialPage) {
        ...
        try {
            ...
            taApp.setLogoutCallback(() -> {
                ...
                // Restore the original Look and Feel (avoid affecting the authentication module).
                try {
                    if (savedLookAndFeel != null && !savedLookAndFeel.isEmpty()) {
                        UIManager.setLookAndFeel(savedLookAndFeel);
                    } else {
    ...
    public void confirmLogout() {
        ...
        if (ok == JOptionPane.OK_OPTION) {
            // Clear cached AI analysis results only on full logout.
