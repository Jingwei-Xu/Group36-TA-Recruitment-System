...
public class PortalNavigator {
    ...
    public void showPage(String pageName) {
        if (PortalRoutes.APPLICATIONS.equals(pageName)) {
            ...
            // Reset scroll before the card is shown so the first paint is already at the top.
            jobsPage.scrollJobsAndAiRankingToTop();
        }
        cardLayout.show(mainContentPanel, pageName);
        if (PortalRoutes.JOBS.equals(pageName)) {
            // One EDT pass after layout; avoid delayed timers that cause a visible jump.
