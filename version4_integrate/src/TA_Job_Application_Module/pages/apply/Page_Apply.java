...
import TA_Job_Application_Module.util.RelevantSkillsHelper;
...
public class Page_Apply {
    ...
    private void buildContent(Job job) {
        ...
        body.add(createLabeledArea("Relevant Skills *",
                "e.g., Java (Intermediate), Python (Beginner)", skillsArea));
        skillsArea.setText(RelevantSkillsHelper.formatSkillsText(currentUser));
    ...
    private Application buildApplicationFromForm(Job job, JTextField fullNameField,
        ...
        if (skillsArea != null) {
            appForm.setRelevantSkills(RelevantSkillsHelper.parseSkillsText(skillsArea.getText()));
        }
    ...
    private void buildContentForDraft(Job job, Application draft) {
        ...
        String skills = form != null && form.getRelevantSkills() != null
                ? RelevantSkillsHelper.formatSkillsText(form.getRelevantSkills())
                : RelevantSkillsHelper.formatSkillsText(currentUser);
        ...
        JTextArea skillsArea = UI_Helper.createTextArea(4);
        body.add(createLabeledArea("Relevant Skills *", "e.g., Java (Intermediate), Python (Beginner)", skillsArea));
    ...
    private Icon miniIcon(Color ink, int size, String kind) {
        return new Icon() {
            ...
            public void paintIcon(Component c, Graphics g, int x, int y) {
                ...
                switch (kind) {
                    case "person" -> {
                        // Draw with proportional geometry to avoid blurring at 18px.
                        float cxf = s / 2f;
                        float headR = s * 0.24f;
                    ...
                    case "phone" -> {
                        // Rounded-rectangle phone outline is clearer than the old flip-phone arcs.
                        float cxf = s / 2f;
                        float bw = s * 0.46f;
                        float bh = s * 0.72f;
                        float x0 = cxf - bw / 2f;
