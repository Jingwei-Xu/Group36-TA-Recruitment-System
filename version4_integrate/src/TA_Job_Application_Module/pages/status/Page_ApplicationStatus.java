...
public class Page_ApplicationStatus {
    ...
    /** Center dialogs against the top-level window; using inner {@link #panel} can mis-center in scroll layouts. */
    ...
    private JPanel buildOfferCard(Application app) {
        ...
        // Keep text and buttons on separate rows: EAST can squeeze CENTER and truncate fixed-width HTML text.
        JPanel content = new JPanel();
        content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS));
        content.setOpaque(false);
        ...
        // Avoid Unicode warning glyphs (Swing HTML may render squares in some environments).
    ...
    /**
     * Offer congratulation text: render HTML in {@link JTextPane} inside a no-scrollbar {@link JScrollPane}.
     * With {@link JTextPane#getScrollableTracksViewportWidth()} true, wrapping follows viewport width and avoids clipping.
     */
