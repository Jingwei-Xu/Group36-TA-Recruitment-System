...
import TA_Job_Application_Module.util.RelevantSkillsTypeAdapter;
...
public class DataService {
    ...
    // AI analysis result cache (persisted in user data).
    ...
    private com.google.gson.JsonObject applicationFormToJson(Application.ApplicationForm af) {
        ...
        if (af.getRelevantSkills() != null) {
            for (Application.RelevantSkill sk : af.getRelevantSkills()) {
                if (sk == null || sk.getName() == null || sk.getName().isBlank()) {
                    continue;
                }
                com.google.gson.JsonObject skillObj = new com.google.gson.JsonObject();
                skillObj.addProperty("name", sk.getName());
                if (sk.getProficiency() != null && !sk.getProficiency().isBlank()) {
                    skillObj.addProperty("proficiency", sk.getProficiency());
                }
        ...
        this.gson = new GsonBuilder()
                ...
                .registerTypeAdapter(
                        TypeToken.getParameterized(
                                List.class,
                                Application.RelevantSkill.class).getType(),
                        new RelevantSkillsTypeAdapter())
                .create();
        loadMockData();
    }
    ...
    public boolean declineOffer(String applicationId) {
        ...
        // Decline offer: set status to Rejected instead of Cancelled.
        app.getStatus().setCurrent("rejected");
        app.getStatus().setLabel("Rejected");
        app.getStatus().setLastUpdated(LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));

        // Do not set isDeleted so the record remains visible.
        if (app.getMeta() != null) {
            app.getMeta().setUpdatedAt(LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
        }
